...
if(isset($_POST['submit'])){
  ...
  if(empty($id)){
    header("Location: sessionDataIn.php?error = id is required");
    exit();
  }else if(empty($name)){
    header("Location: sessionDataIn?error = Name is required");
    exit();
  }else{
     $sql = "insert into session(sessionid,sessionname)
           values('$id','$name')";
       $result = mysqli_query($con, $sql);
  if($result){
    header("Location: sessionData.php");
  }else{
    header("Location: sessionData.php?error = Primary key voilation");
  }
}
}
?>
    ...
    <div class="container my-5">
        ...
        <form method="post">
            <div class="form-group">
                <label class="title">Add Session Form</label>
            </div>
            <div class="form-group">
