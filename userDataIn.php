...
if(isset($_POST['submit'])){
  ...
  if(empty($name)){
    header("Location: userDataIn?error = Name is required");
    exit();
  }else if(empty($password)){
    header("Location: userDataIn.php?error = Password is required");
    exit();
  }else if(empty($email)){
    header("Location: userDataIn.php?error = Email is required");
    exit();
  }else if(empty($phone)){
    header("Location: userDataInr.php?error = Phone number is required");
    exit();
  }else if(empty($city)){
    header("Location: userDataIn.php?error = City is required");
    exit();
  }else if(empty($postal)){
    header("Location: userDataIn.php?error = Postal is required");
    exit();
  }else if(empty($house)){
    header("Location: userDataIn.php?error = House is required");
    exit();
}else if(empty($house)){
    header("Location: userDataIn.php?error = House is required");
    exit();
    ...
    exit();
     ...
     $sql = "insert into user(userEmail,name,password,phoneNum,postacode,city,streetID,houseNum,sessionid,packageid,classid,trainerEmail)
           values('$email','$name','$password','$phone','$postal','$city','$street','$house','$sessionid','$packageid','$classid','$trainerEmail')"; //user insert query
     $result = mysqli_query($con, $sql);
  if($result){
    $planname = "Unavailabe";
    ...
    header("Location: userData.php?error = Constraint voilation");
  }
}
}
?>
