...
if(isset($_POST['submit'])){
  ...
  if(empty($userEmail)){
    header("Location: userPaymentDataIn.php?error = User email is required");
    exit();
  }else if(empty($status)){
    header("Location: userPaymentDataIn?error = Status is required");
    exit();
  }else if(empty($date)){
    header("Location: userPaymentDataIn?error = Date is required");
    exit();
  }else{
     $sql = "insert into userPayment(userEmail,status,date)
           values('$userEmail','$status','$date')";
       $result = mysqli_query($con, $sql);
  if($result){
    header("Location: userPaymentData.php");
  }else{
    header("Location: userPaymentData.php?error = Primary key voilation");
  }
}
}
?>
    ...
    <div class="container my-5">
        ...
        <form method="post">
            <div class="form-group">
                <label class="title">Add Fitness Form</label>
            </div>
            <div class="form-group">
