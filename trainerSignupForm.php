...
if(isset($_POST['submit'])){
  ...
  if(empty($email)){
    header("Location: trainerSignupForm.php?error = Email is required");
    exit();
   }else if(empty($name)){
    header("Location: trainerSignupForm.php?error = Name is required");
    exit();
  }else if(empty($password)){
    header("Location: trainerSignupForm.php?error = Password is required");
    exit();
  
  }else if(empty($phone)){
    header("Location: trainerSignupForm.php?error = Phone number is required");
    exit();
  }else if(empty($city)){
    header("Location: trainerSignupForm.php?error = City is required");
    exit();
  }else if(empty($postal)){
    header("Location: trainerSignupForm.php?error = Postal is required");
    exit();
  }else if(empty($house)){
    header("Location: trainerSignupForm.php?error = House is required");
    exit();
  }else{
  ...
  if($result){
    ...
    header("Location: allLogin.php?error = Email is already used");
  }
}
}
?>
...
<body>
    ...
    <div class="container my-5">
        ...
        <form method="post">
            ...
            <button name="submit" class="insertbtn">Sign up</button>
        </form>
