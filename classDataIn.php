...
if(isset($_POST['submit'])){
  ...
  if(empty($id)){
    header("Location: classDataIn.php?error = id is required");
    exit();
  }else if(empty($name)){
    header("Location: classDataIn?error = Name is required");
    exit();
  }else{
  ...
  if($result){
    ...
    header("Location: classData.php?error = Primary key voilation");
  }
}
}
?>
    ...
    <div class="container my-5">
        ...
        <form method="post">
            <div class="form-group">
                <label class="title">Add class Form</label>
            </div>
            <div class="form-group">
