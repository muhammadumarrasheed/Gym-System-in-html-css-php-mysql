...
if(isset($_POST['submit'])){
  ...
  if(empty($userEmail)){
    header("Location: fitnessDataIn.php?error = User email is required");
    exit();
  }else if(empty($status)){
    header("Location: fitnessDataIn?error = Status is required");
    exit();
  }else{
  ...
  if($result){
    ...
    header("Location: fitnessData.php?error = Primary key voilation");
  }
}
}
?>
