...
if(isset($_POST['submit'])){
  ...
  $trainerEmail= validate($_POST['trainerEmail']);
  $status = validate($_POST['status']);
  $date = $_POST['date'];

  if(empty($trainerEmail)){
    header("Location: trainerPaymentDataIn.php?error = trainer email is required");
    exit();
  }else if(empty($status)){
    header("Location: trainerPaymentDataIn?error = Status is required");
    exit();
}else if(empty($date)){
    header("Location: trainerPaymentDataIn?error = Date is required");
    exit();
  }else{
     $sql = "insert into trainerPayment(trainerEmail,status,date)
           values('$trainerEmail','$status', '$date')";
       $result = mysqli_query($con, $sql);
  if($result){
    header("Location: trainerPaymentData.php");
  }else{
    header("Location: trainerPaymentData.php?error = Foreign key voilation");
  }
}
}
?>
...
<head>
    ...
    <title>trainer payment add</title>
</head>
<style>
* {
    ...
    <div class="container my-5">
        ...
        <form method="post">
            <div class="form-group">
                <label class="title">Add trainer payment Form</label>
            </div>
            <div class="form-group">
                <label class="form-label">Trainer Email</label>
                <input type="email" class="form-control" placeholder="Enter email" name="trainerEmail"
                    autocomplete="off">
            </div>
