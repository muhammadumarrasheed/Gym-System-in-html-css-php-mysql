<?php
include 'dbconn.php';
$userEmail = $_GET['updname'];
$sql = "select * from `fitness` where userEmail = '$userEmail'";
$result = mysqli_query($con,$sql);
$row = mysqli_fetch_assoc($result);
$status = $row['fitnessstatus'];
if(isset($_POST['submit'])){
$status = $_POST['status'];
$sql = "update `fitness` set userEmail = '$userEmail',fitnessstatus = '$status' where userEmail = '$userEmail'";
$result = mysqli_query($con, $sql);
if($result){
        header("Location: fitnessData.php");
}else{
header("Location:  fitnessData.php?error = Data is not updated");
}
}?>
<!doctype html>
<html lang="en">
    ...
    <div class="container my-5">
        <form method="post">
            <div class="form-group">
                <label class="title">Fitness update Form</label>
            </div>
            <div class="form-group">
                <label class="form-label">Status</label>
                <input type="text" class="form-control" placeholder="Enter status" name="status" autocomplete="off"
                    value=<?php
                echo $status?>>
            </div>
