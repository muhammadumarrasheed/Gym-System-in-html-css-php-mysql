...
$sql = "select * from `plan` where userEmail = '$userEmail'";
$result = mysqli_query($con,$sql);
$row = mysqli_fetch_assoc($result);
$name = $row['planname'];
if(isset($_POST['submit'])){
$name = $_POST['name'];
$sql = "update `plan` set userEmail = '$userEmail',planname = '$name' where userEmail = '$userEmail'";
$result = mysqli_query($con, $sql);
if($result){
        header("Location: planData.php");
}else{
header("Location:  planData.php?error = Data is not updated");
}
}?>
<!doctype html>
<html lang="en">
    ...
    <div class="container my-5">
        <form method="post">
            <div class="form-group">
                <label class="title">Plan update Form</label>
            </div>
            <div class="form-group">
                <label class="form-label">Plan</label>
                <input type="text" class="form-control" placeholder="Enter plan" name="name" autocomplete="off" value=<?php
                echo $name?>>
            </div>
