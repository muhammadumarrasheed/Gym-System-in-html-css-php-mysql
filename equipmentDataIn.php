...
if(isset($_POST['submit'])){
  ...
  if(empty($id)){
    header("Location: equipmentDataIn.php?error = id is required");
    exit();
  }else if(empty($name)){
    header("Location: equipmentDataIn?error = Name is required");
    exit();
  }else{
     $sql = "insert into equipment(id,name)
           values('$id','$name')";
       $result = mysqli_query($con, $sql);
  if($result){
    header("Location: equipmentData.php");
  }else{
    header("Location: equipmentData.php?error = Primary key voilation");
  }
}
}
?>
    ...
    <div class="container my-5">
        ...
        <form method="post">
            <div class="form-group">
                <label class="title">Add Equipment Form</label>
            </div>
            <div class="form-group">
