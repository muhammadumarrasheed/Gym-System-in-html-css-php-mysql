...
if(isset($_POST['submit'])){
  ...
  if(empty($id)){
    header("Location: packageDataIn.php?error = id is required");
    exit();
  }else if(empty($name)){
    header("Location: packageDataIn?error = Name is required");
    exit();
  }else{
     $sql = "insert into package(packageid,packagename)
           values('$id','$name')";
       $result = mysqli_query($con, $sql);
  if($result){
    header("Location: packageData.php");
  }else{
    header("Location: packageData.php?error = Primary key voilation");
  }
}
}
?>
    ...
    <div class="container my-5">
        ...
        <form method="post">
            <div class="form-group">
                <label class="title">Add Package Form</label>
            </div>
            <div class="form-group">
